...
public class ModMenuIntegration implements ModMenuApi {
	...
	public ConfigScreenFactory<?> getModConfigScreenFactory() {
		return parent -> {
			...
			Option<Boolean> showEffectNamesOpt = Option.<Boolean>createBuilder()
					...
					.binding(false, () -> ModConfig.get().showEffectNames, val -> ModConfig.get().showEffectNames = val)
					.controller(opt -> BooleanControllerBuilder.create(opt).yesNoFormatter())
					.available(ModConfig.get().replaceEffects)
					.build();

			Option<Boolean> replaceEffectsOpt = Option.<Boolean>createBuilder()
					.name(Component.translatable("text.autoconfig.numericalhud.option.replaceEffects"))
					.description(OptionDescription.of(Component.translatable("text.autoconfig.numericalhud.option.replaceEffects.@Tooltip")))
					.binding(true, () -> ModConfig.get().replaceEffects, val -> ModConfig.get().replaceEffects = val)
					.controller(opt -> BooleanControllerBuilder.create(opt).yesNoFormatter())
					.listener((opt, val) -> showEffectNamesOpt.setAvailable(val))
					.build();

			Option<Boolean> replaceBossBarOpt = Option.<Boolean>createBuilder()
					.name(Component.translatable("text.autoconfig.numericalhud.option.replaceBossBar"))
					.description(OptionDescription.of(Component.translatable("text.autoconfig.numericalhud.option.replaceBossBar.@Tooltip")))
					.binding(true, () -> ModConfig.get().replaceBossBar, val -> ModConfig.get().replaceBossBar = val)
					.controller(opt -> BooleanControllerBuilder.create(opt).yesNoFormatter())
					.build();
			...
			return YetAnotherConfigLib.createBuilder()
					...
					.category(ConfigCategory.createBuilder()
							...
							.build())
					.category(ConfigCategory.createBuilder()
							.name(Component.literal("Others"))

							.option(replaceBossBarOpt)
							.option(replaceEffectsOpt)
							.option(showEffectNamesOpt)
