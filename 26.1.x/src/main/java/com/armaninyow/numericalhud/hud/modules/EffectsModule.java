...
public class EffectsModule extends BaseHudModule {
	...
	public void render(GuiGraphicsExtractor context, Player player, int x, int y, float tickDelta) {
		if (!ModConfig.get().replaceEffects) return;
		...
		for (int i = 0; i < visible.size(); i++) {
			...
			int nameColor  = applyAlpha(effect.isAmbient() ? AMBIENT_COLOR : TEXT_COLOR, textAlpha);
			int timerColor = applyAlpha(TEXT_COLOR, textAlpha);

			boolean showNames = ModConfig.get().showEffectNames;
