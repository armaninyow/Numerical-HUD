...
public abstract class BaseHudModule {
	...
	/**
	 * Subclasses must supply a version-specific icon renderer.
	 * Each version subproject provides its own implementation class.
	 */
	...
	protected void drawIcon(DrawContext context, Identifier texture, int x, int y, int color) {
		getIconRenderer().drawIcon(context, texture, x, y, ICON_SIZE, color);
	}

	/**
	 * Draws a 75% white overlay on top of the given texture to simulate a blink flash.
	 * Use this instead of a separate blinking texture asset.
	 * 0xBF = 191 = ~75% of 255 alpha, 0xFFFFFF = full white RGB.
	 */
	protected void drawBlinkOverlay(DrawContext context, Identifier texture, int x, int y) {
		getIconRenderer().drawIcon(context, texture, x, y, ICON_SIZE, 0xBFFFFFFF);
	}

	/**
	 * Draws a vanilla GUI atlas sprite (e.g. Identifier.of("minecraft", "hud/armor_full")).
	 * Use this for all vanilla HUD sprites instead of drawIcon.
	 */
	protected void drawVanillaSprite(DrawContext context, Identifier sprite, int x, int y) {
		getIconRenderer().drawVanillaSprite(context, sprite, x, y, ICON_SIZE, 0xFFFFFFFF);
	}

	/**
	 * Draws a 9x5 slice of the XP bar texture (column 0 + columns 174-181).
	 */
	protected void drawVanillaXpBar(DrawContext context, Identifier sprite, int x, int y) {
		getIconRenderer().drawVanillaXpBar(context, sprite, x, y);
	}

	/**
	 * Draws a 9x5 slice of a jump bar texture (same 182x5 layout as the XP bar).
	 * rightEnd=false → col 0 + cols 1–8   (blue / background region)
	 * rightEnd=true  → col 0 + cols 174–181 (orange / full-charge region)
	 */
	protected void drawVanillaJumpBar(DrawContext context, Identifier sprite, int x, int y, boolean rightEnd) {
		getIconRenderer().drawVanillaJumpBar(context, sprite, x, y, rightEnd);
	}

	/**
	 * Flushes buffered draws. Call once after all modules have rendered.
	 */
	...
	private void drawTextOutline(DrawContext context, MinecraftClient client, String text, int x, int y, int color) {
		// Derive outline color by halving each RGB channel of the text color
		int a  = (color >> 24) & 0xFF;
		...
		context.drawText(client.textRenderer, text, x,     y + 1, outlineColor, false);
		context.drawText(client.textRenderer, text, x + 1, y + 1, outlineColor, false);
		// Draw text on top in its actual color
